...
* **`auth`**: **「誰として通すか」の契約と合成**（標準ライブラリのみ）
  ...
  * **応答を決めるのは、資格情報を提示したうえで落ちた方式です。** Bearer を出して通らなかった
    エージェントは 403 を受け取り、何も出していないブラウザはログイン画面へ送られます。
    最後の方式に答えさせると、**JSON を求めたエージェントに HTML のログイン画面が返ります**。

| 状況 | `auth.Protected(verifier, session)` | `auth.Require(verifier)` |
| --- | --- | --- |
| 資格情報なし（ブラウザ） | 302 ログイン画面へ | 401 |
| Bearer を出したが通らない | 403 | 403 |
| 検証器が未設定 | 302 ログイン画面へ（ログに記録） | 500 |
