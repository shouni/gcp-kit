...
* **`auth/session`**: **Google OAuth2 ログインとセッション・CSRF**
  ...
  * **ログイン時にセッション ID を振り直します**（セッション固定攻撃対策）。Cookie ストアでは
    クッキー自体が中身なので影響ありませんが、サーバーサイドのストアではクッキーが運ぶのは
    ID だけです。**攻撃者が事前に仕込んだ ID をそのまま認証済みにすると、その ID で被害者として
    振る舞えます。**
  * **ログアウトを効かせるには `WithPrompt` が要ります**: `Logout` が消せるのはこのアプリの
    クッキーだけで、Google 側のセッションは残ります。既定ではログイン画面へ送った時点で
    Google が何も聞かずに承認を返すため、**一瞬ちらついて元のログイン状態に戻る**だけです
    （共用端末で「ログアウト」が効きません）。`WithPrompt(session.PromptSelectAccount)` を
    渡すとアカウント選択が必ず挟まります。Google 自体からのログアウトはしません
    （Gmail など全サービスから追い出すことになるため）。
...
* **`cloudlog`**: **Cloud Logging 互換の構造化ログ**
  ...
  * **トレース相関**: `TraceMiddleware` が `X-Cloud-Trace-Context` を解析し、リクエスト単位で
    ...
    * **表現を Cloud Logging 側へ揃えます**: ヘッダー上の `SPAN_ID` は 10 進数ですが、
      `spanId` フィールドが期待するのは 16 進 16 桁です。生のまま載せるとスパンと
      突き合わせられないため、`ParseTraceContext` が変換して返します。
    * **形式に合わないヘッダーは相関に使いません**: このヘッダーは呼び出し元が自由に
      付けられ、Cloud Run は提示された値を引き継ぎます。検証せずに予約フィールドへ
      書くと、任意の文字列を仕込む入口になり、他リクエストのトレースへ相乗りもできます。
  * **組み立ては `NewHandler` が持ちます**: `slogctx` で包み忘れても**エラーは出ず、
    `job_id` やトレース ID がログから消えるだけ**なので、6 つのアプリが同じ 3 行を
    逐語で写していました。落ちない失敗はコピーで守れません。
...
* **`negotiate`**: **通した相手に合わせて表現を選ぶ**（`auth` と対になります）
  ...
  * **書き出しも持ちます。** `JSON` は `Content-Type` と状態コードを整えて payload を書き、
    `Error` は相手が JSON を求めていれば `{"error": ...}` を、そうでなければ `text/plain` を
    返します（画面側の JS がエラー本文を `resp.text()` で読むため、JSON 固定にはしません）。
    判定だけを共有して書き出しを各アプリに残した結果、`Content-Type` の `charset` の有無、
    エンコード失敗の扱い（context 付きログ / context 無し / 握り潰し）が割れていました。
